...
class HomeCalendar extends React.Component {
  state = {
    eventIds: [],
    first_color: this.props.currentUser.first_color
      ? this.props.currentUser.first_color
      : 'pink',
    second_color: this.props.currentUser.second_color
      ? this.props.currentUser.second_color
      : 'purple',
    visible: false,
  };
  ...
  onClick = (e) => {
    ...
    if (eventIds.includes(parseInt(e.target.id))) {
      ...
      let indexOfDeleted = updatedList.findIndex(
        (eventId) => eventId === parseInt(e.target.id),
      );
      updatedList.splice(indexOfDeleted, 1);
      this.setState({ eventIds: updatedList });
    } else {
      this.setState({
        eventIds: [...eventIds, parseInt(e.target.id)],
      });
    }
  };
      ...
      return {
        title:
          event.user_id === currentUser.id
            ? event.title
            : `${event.initials} - ${event.title}`,
        bgColor:
          event.user_id === currentUser.id
            ? currentUser.first_color
              ? currentUser.first_color
              : 'pink'
            : currentUser.second_color
            ? currentUser.second_color
            : 'purple',
        start: moment(event.date),
        end: moment(event.date),
        ...
        >
          <div>
            {followedEvents.length > 0 && eventIds.length > 0 ? (
              ...
              >
                click icons to toggle events off calendar
              </Label>
            )}
          </div>
          <div
