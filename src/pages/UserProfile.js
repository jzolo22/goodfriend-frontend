...
class UserProfile extends React.Component {
  state = {
    ...
    partner_birthday: moment(
      this.props.user[0].partner_birthday,
    ).format('MMM Do'),
    visible: true,
    src: null,
    crop: {
      ...
      if (user[0].id === currentUser.id) {
        ...
        return (
          <p style={{ fontSize: '20px', marginBottom: '5px' }}>
            🎂 {moment(user[0].birthday).format('MMM Do')}
          </p>
        );
      }
    }
      ...
      if (user[0].id === currentUser.id) {
        ...
        return (
          <p style={{ marginBottom: '3px' }}>
            {' '}
            💞 {user[0].partner_name} - 🎂{' '}
            {moment(user[0].partner_birthday).format('MMM Do')}
          </p>
        );
      }
  ...
  editedPartnerBirthday = (text) => {
    ...
    this.setState(
      ...
      () =>
        this.props.editProfile(userId, {
          partner_birthday: newBirthdayDate,
        }),
    );
  };
  ...
  eventDots = () => {
    ...
    if (user[0].own_events.length > 0) {
      let usersEvents = allEvents.filter(
        (event) => event.user_id === user[0].id,
      );
      let sortedByDateEvents = usersEvents.sort(
          ...
          <Segment raised>
            {user[0].id === currentUser.id ? (
              <>
                ...
                {this.state.cropBtn ? (
                  <Button onClick={this.submitCrop}>
                    Done Cropping
                  </Button>
                ) : null}
              ...
              <>
                <Image
                  ...
                  src={user[0].profile_picture.url}
                  circular
                  size="small"
                />
                <p
...
const mdp = (dispatch) => {
  return {
    ...
    editPicture: (userId, userObj) =>
      dispatch(editPicture(userId, userObj)),
  };
};
