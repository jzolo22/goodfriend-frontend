...
class Wishlist extends React.Component {
  ...
  wishlistItems = () => {
    if (this.props.items.length > 0) {
      const usersWishlistItems = this.props.items.filter(
        (item) => item.wishlist_id === this.props.user.wishlist.id,
      );
            ...
            <OuterContainer>
              <StyledImage
                src={
                  item.image_link
                    ? item.image_link
                    : 'https://www.nashvillewraps.com/blog/wp-content/uploads/2014/02/Wedding-editorial1.jpg'
                }
                alt={item.name}
              />
                      ...
                      children: (
                        <a
                          style={{
                            ...
                            fontWeight: 'bold',
                            fontSize: '15px',
                            textDecoration: item.purchased ? 'line-through' : null,
                          }}
                          target="_blank"
                          rel="noreferrer"
                          href={item.link}
  ...
  render() {
    return (
      <>
        <Container>{this.wishlistItems()}</Container>
