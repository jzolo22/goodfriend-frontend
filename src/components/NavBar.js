...
class NavBar extends React.Component {
  ...
  matchingNames = (searchInput) => {
    ...
    if (this.props.allUsers.length > 0) {
      results = this.props.allUsers.filter(
        (user) =>
          user.first_name
            .toLowerCase()
    ...
    return (
      <>
        ...
        <Menu fixed="top" inverted={true} size="mini">
          ...
          <Menu.Item as={NavLink} to={'/'} name="goodFriend">
            <Item.Content>
              <img
                src="/images/goodfriendwhitetransparent.png"
                style={{ width: '56px' }}
                alt="goodfriend logo"
              />
            </Item.Content>
              ...
              >
                <Item.Content>
                  {this.props.currentUser.profile_picture ? (
                    <Image
                      src={this.props.currentUser.profile_picture.url}
                      circular
                      size="mini"
                    />
                  ) : null}
